    ...
    if( downloadState == DOWNLOAD_FILE_NONEXIST)
    {
        ...
        while((sockfd = connectFtpServer(ftpServerIP, port, username, passwd) )<= FTP_CONNECT_FAILED_FLAG)
        {
            if( MAX_CONNECT_TIMES <= ++conncectTimes )
            {
                ...
                writeLog("%s\t%s\t%s\t%s\n",startTime, "DOWNLOAD_CONNNET_FAILED", remoteFullPath, "");
        ...
        if( sockfd > FTP_CONNECT_FAILED_FLAG )
        {
            ...
            switch(ftperror)
            {
                case DOWNLOAD_CONNNET_FAILED:
                {
                    ...
                    #endif
                    writeLog("%s\t%s\t%s\t%s\n",startTime, "DOWNLOAD_CONNNET_FAILED", remoteFullPath, endTime);
                ...
                case DOWNLOAD_REMOTE_FILE_NOEXIST:
                {
                    ...
                    #endif
                    writeLog("%s\t%s\t%s\t%s\n",startTime, "DOWNLOAD_FAILED", remoteFullPath, endTime);
                    break;
                }
                case FTP_DOWNLOAD_SUCCESS:
                {
                    ...
                    #endif
                    writeLog("%s\t%s\t%s\t%s\n",startTime, "DOWNLOAD_SUCCESS", remoteFullPath, endTime);
...
void download()
{
    ...
    while(1)
    {
        ...
        while( p != NULL )
        {
            ...
            {
                #ifdef DEBUG
                printf("Fun(download):p->state is NULL.\n");
                #endif
                continue;
            }
