...
void sigleDownload(DownloadNode *dwNode, int nodeid)
{
	...
	len = strlen(dwNode->localPath) + 1;
	localDir = (char *)malloc(len);
	memset(localDir, 0, len);
...
void printTask(DownloadNode * p)
{
    ...
    sprintf(buffer,"%s\t%s\t%s\t%d\t%s\t%d\t%s\t%s\t\n",p->filename,p->localPath,p->remotePath,p->taskNum, p->server->ip,  p->server->port, p->server->username, p->server->passwd );
    plog(buffer);
    ...
    while(1)
    {
        ...
        while( p != NULL )
        {
            ...
            */
            int j = 0;
            pthread_t *mp;
            int realTaskThreadMem = sizeof(pthread_t)*taskNum;
            mp = (pthread_t *)malloc(realTaskThreadMem);
            memset(mp,0,realTaskThreadMem);
